...
`AbstractRepository`, we can unit test with `FakeRepository`, but run it our production code using `SqlAlchemyRepository`.
...
codes, and JSON. All the orchestration logic is in the use -ase/service layer,
...
cart application, you might choose to build taxation rules as a _domain service_.
...
* When combined with the _Repository pattern_ and `FakeRepository`, we have
