...
<4> Entrypoints are the places we drive our application from.  Arguably these are 
    adapters too.  Upstream adapters are called "primary" adapters, and downstream
    adapters (like the SQL repository) are called "secondary" adapters.

What about ports?  If you really wanted to go to town, you could split out the
abstract repository (which is the Port) into its own file.  Bob likes to say
that the domain owns the repository:
