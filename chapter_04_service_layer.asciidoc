...
* We've decoupled our domain model classes from the client code that calls our
  application, which makes us more free to refactor.

* If you've ever found yourself wanting to have one of your web controllers (or
  Django views) call another one, and you've felt dirty about it, the Service Layer
  would let you do that in a structured way.

* When combined with _Repository Pattern_ and a `FakeRepository`, we've got
  a nice way of writing unit tests at a higher level than the Domain Layer;
...
* If you do ever want to share some logic between two controllers, you can
  always extract a function just for that case; there's no need to rewrite
  your entire application the very first time you see a need for reuse
