...
Back to our allocations project! <<maps_service_layer_before>> shows the point we reached at the end of <<chapter_02_repository>>, which covered the Repository pattern.((("service layer", id="ix_serlay")))
...
in front of the users to start gathering feedback.((("service layer", "connecting our application to real world"))) We have the core
...
an integration test versus a unit test.((("testing", "end-to-end test of allocate API")))((("end-to-end tests", "of allocate API")))((("APIs", "end-to-end test of allocate API"))) Different projects need different
...
* You can get a lot of the benefits that come from having rich domain models
  ...
  controllers").((("service layer", startref="ix_serlay")))
|===
