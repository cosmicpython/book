...
collecting and processing new events.

We make a couple of modification to the main `messagebus.handle()` function:
...
<4> The messagebus passes a UoW down to each handler
<5> After each handler, we collect any new events that have been generated,
    and we add them to the queue.

// TODO: add diff? and/or do in two stages, add uow first, then add queue?
...
.UoW passes self to message bus (src/allocation/service_layer/unit_of_work.py)
====
[source,python]
----
