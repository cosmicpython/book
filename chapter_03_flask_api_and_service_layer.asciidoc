...
* Show how expressing our service layer functions in terms of
  primitives allows the service-layer's users (our tests and our
  flask API) to be decoupled from the model layer.
...
//TODO (DS) - not entirely clear what invalid means or how that test works
...
Alongside `FakeRepository`, we also build `FakeSession` to fake out the database
session. (The fake session is only a temporary solution.  We'll get rid of
it and make things even nicer in the next chapter, <<chapter_04_uow>>)
