...
the domain-level tests there for now, as documentation, but if we ever find
them to be constraining, if we find ourselves implementing new functionality
that involves changes to our model that breaks all these tests, then we'd
want to rewrite all of the ones that are still valid in terms of the service
layer instead.

But we still have some direct dependencies on the domain in our service-layer
...
How would it look if it only took primitives?
...
And we can rewrite the tests in those terms as well:
