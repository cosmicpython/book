...
// still some more stuff we can tweak:
...
TIP: Wherever possible, write your unit tests at the highest possible level
    of abstraction.  The lower level a unit test is, the more constraining
    it is against future refactoring

.High Gear vs Low Gear,
*******************************************************************************

write tests at highest possible level, drop down to lower level when things
get harder.  high gear vs low gear analogy.

in this case we started in low gear and moved up to high gear.  that's good too.

TODO: flesh out this bit.

*******************************************************************************

So we could choose to do that at this point, or (more likely), we can leave
the domain-level tests there for now; they serve as documentation, if nothing
else. But if we ever find them to be constraining, if we find ourselves
implementing new functionality that involves changes to our model that breaks
all these tests, then we'd want to rewrite them all in terms of the service
layer instead--the ones that are still valid, in any case.
...
This is a really nice place to be in.  Our service-layer tests only
depend on the services layer itself, leaving us completely free to
refactor the model as we see fit.


//TODO: add add_stock api endpoint?
