...
// (EJ2) Suggest recasting this contrast to "composition in a functional vs. object-oriented style."
// Mentioning FCIS is still fine in Ch3.
...
// (EJ3) Also consider using "Inversion of Control" instead of DIP in some places.
// (They are very closely related, so the the distinction is a little academic for the purpose of the book.)
...
frameworkfootnote:[Mark Seeman calls this https://blog.ploeh.dk/2012/11/06/WhentouseaDIContainer/[Pure DI].]).
...
appeal to other people.   It requires rewriting all our handler functions as
classes though:
...
////
(EJ2) re: hynek's comment, there is a valuable point.  Composition by injecting a class
    is more explicit.  But it's also more verbose, and sometimes excessive for very small
    code fragments.  IMHO which one to use is a judgement call, and in either case you should
    be mindful of whether the code is more or less maintainable as a result.
///
