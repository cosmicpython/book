...
Our bootstrapper will do any required application initialisation,
...
def bootstrap(
    start_orm: bool = True,
    session_factory: SessionFactoryType = DEFAULT_SESSION_FACTORY,
...
<1>  `orm.start_mappers()` is our example of initialisation work that needs
...
<3> and we initialize the message bus for the application.



[[di_by_inspection]]
.DI by inspecting function signatures
====
[source,python]
----
...
+            handler = self.command_handlers[type(command)]  #<2>
+            handler(command)  #<2>
         except Exception:
...
<2> and instead of passing a UoW into the handler, they expect the handlers
    to already have all their dependencies, and only need a specific event or
    command.
