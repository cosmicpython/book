...
NOTE: At some point, regardless of tenacity, we'll have to give up trying to
process the message. Building reliable systems with distributed messages is
hard. There's pointers to reference materials in the next chapter and the
epilogue.
...
// TODO (EJ1) I don't think you've ever mentioned how to handle a failure of
// the version number check for concurrency control. Might want to mention
// how/where that retry would be handled.
