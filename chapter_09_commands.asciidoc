...
[[handle_command]]
.Commands reraise exceptiosn (src/allocation/service_layer/messagebus.py)
====
[source,python]
----
...
[[flask_uses_command]]
.Flask gets a response from the command handler (src/allocation/entrypoints/flask_app.py)
====
