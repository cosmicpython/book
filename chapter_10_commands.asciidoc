...
This might have felt counterintuitive. After all, the batch _hasn't_ been
...
https://github.com/cosmicpython/code/tree/chapter_10_commands[chapter_10_commands] branch on GitHub.
...
Just changing ((("events", "commands versus", startref="ix_evntcmd")))the names and verb tenses is all very well, but that won't
...
event fails to process? ((("error handling", "events, commands, and", id="ix_errhnd")))((("commands", "events, commands, and error handling", id="ix_cmndeverr")))How am I supposed to make sure the system is in a
consistent state?"
...
cause an inconsistent state: the three units of stock are both allocated and
...
complex.((("error handling", "events, commands, and", startref="ix_errhnd")))((("commands", "events, commands, and error handling", startref="ix_cmndeverr")))
...
WARNING: At some point, regardless of tenacity, we'll have to give up trying to
    ...
    reference materials in <<epilogue_1_how_to_get_there_from_here>>.
