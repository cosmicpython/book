...
// TODO (DS) does the messagebus "handle" events? isnt that the handlers?  maybe
// "routes"?
// (EJ2) messagebus "dispatches" events makes sense to me and is unambiguous.

[TIP]
====
You can find our code for this chapter at
https://github.com/cosmicpython/code/tree/chapter_07_events_and_message_bus[github.com/cosmicpython/code/tree/chapter_07_events_and_message_bus].
...
git checkout chapter_06_aggregate
----
====
...
//(EJ1) Suggestion: Consider using the terms "orchestration" vs "choreography" will help
//      in this chapter to contrast the imperative vs event based approaches.
...
// TODO (hynek) Dunno if you wanna go there but we've got TypedDict in 3.8 now
...
// TODO (EJ1)  I think this NOTE is conflating a couple concepts. 
// 
//        I'd argue that the messagebus is actually async, and you
//        absolutely want to defer processing until later in time, 
//        (after the current message handler is completed.) 

//        The deferred processing lets you reason about the
//        handlers behavior more easily since you have to process
//        a single message to completion.
//
//        May be better to rephrase as:
//       "Note that the messagebus as implemented isn't intended to support
//        concurrent or parallel execution, we're only aiming to separate tasks 
//        conceptually.  See <<celery_sidebar>>."
//
...
// TODO (EJ1) The messagebus has more in common with a node.js, a UI event loop, or an actor framework.

// (hynek) preceding sentence is completely superflous yet so funny that i love
// it so much that i want to marry it.
...
// TODO (hynek) there's enough space to write out SRP
