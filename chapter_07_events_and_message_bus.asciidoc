...
a|
* Event-driven workflows can be confusing because there's often no single place
  in the system where you can understand how a request will be fulfilled.
* The message bus implementation here is synchronous, so our event handlers will
  run in-band with web requests. Adding asynchronous processing is possible but
  makes things even *more* confusing.
* The unit of work raises events for us which is _neat_ but also magic. It's not
