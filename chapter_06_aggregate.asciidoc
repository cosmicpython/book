...
same time because there's no invariant that covers them both.
...
I don't want to modify multiple baskets in a transaction, because there's no
...
eery operation ends in a consistent state. This helps us to reason about our
...
Neither of these concepts really satisfies us, though. I should be able to
