...
In this first part of the book we'll look at how to build a rich object model
through TDD (in <<chapter_01_domain_model,Chapter 1>>), and then we'll see how
to keep that model decoupled from technical concerns. We'll see how to build
...
yet!  We'll introduce each box in the figure, one by one.
