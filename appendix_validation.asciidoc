...
// TODO hynek doesn't like the inline call to json.loads
...
Oof so both this and the Fowler article are full of non-sequiturs.

For one: Postel was wrong. We see it every day. Postel’s law is why the
internet if fucked in so many ways. We should’ve been conservative at receiving
from the get go.Then maybe we wouldn’t have to support 20 variants of
everything, just because some stupid server supported it in 1995.

Secondly and more concretely: ignoring extra keys makes the system not more
robust. It makes it more brittle and confusing for the future. Two random
examples:

1. You have an optional field abc and the user passes abcc. You ignore it, the
user is surprised why shit doesn’t work how they thought it will because you
don’t help them with an error message.

2. Users accidentally pass a field abc=True (maybe copy pasta who knows). It
works well for years und you introduce a field abc that deletes an account or
transfers money around. Boom, stuff broke over night.

The broader your feature envelope, the more nightmares await you down the road.
////
...
// TODO (hynek) an even better argument is that invalid data wanders through
// your system is a time bomb, and the deeper it gets the more damage it can
// do, and the less reasonable tools you have to respond to it.
...
TIP: Usually when you receive an invalid message there's little you can do but
    ...
    see spikes in the numbers of bad messages. Ignoring unrecognised fields and
    being liberal about the format of data helps to keep us robust when things
    change.


// TODO (hynek) hynek dont like this neither
