...
////
(ej) In a miscroservices architecture This gets even harder when there's distributed state
...
////
 (ej): The preceding and following paragraph are a bit confusing until you get to the actual code examples.
...
////
 (ej)   This section opens up a whole can of worms, but here are some thoughts.  They may be
...
(ej) +1 on the where the 4 came from.  Here is a small pseudo-code 
