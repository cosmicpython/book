...
What's the point of a domain model anyway? Why don't we just run everything in
a spreadsheet? Many of our users would be delighted by that. Business users
_like_ spreadsheets for their flexibility and uniform interface.

An enormous number of businesses operate by manually sending spreadsheets back
and forward over e-mail. This "csv over http" architecture has low complexity
but tends not to scale very well because it's difficult to apply constraints.
...
no batch can have negative available stock. Every time we make a change to the
state of the system, our code needs to ensure that we don't break the
invariants.

=== Invariants and Concurrency

In a single threaded single user application it's easy for us to maintain the
invariants of our system.
