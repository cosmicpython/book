...
`SELECT FOR UPDATE` is a way of declaring, during the read phase of
your transaction, that certain rows are going to be updated later,
and other transactions should keep clear.  So this is an example
of pessimistic concurrency control.

Here's how you can use the SQLAlchemy DSL to specify at query-time
that you are marking some rows for update:
...
Again, in our simple case, it's not obvious which to prefer.  In a more complex
scenario, `SELECT FOR UPDATE` might lead to more deadlocks, while `REPEATABLE
READ` would lead to more retries.  But if you have a test like the
one we've shown, you can specify the behavior you want, and see how
it changes.  You can also use the test as a basis for performing
...
Those of you with particularly good memories will remember this diagram from the
start of the book.
...
Well, here we are, at the end of part one. What have we achieved? We've seen how
to start building a domain model that's exercised by a set of high-level unit
tests. Our tests describe the behaviour of our system - the rules that we agreed
with our business stakeholders - in human readable prose. When our business
requirements change, we have confidence that our tests will help us to prove the
new functionality, and when new develoeprs join the project, they can read our
tests to understand how things work.
...
This helps us to keep our codebase well organised, and stops us from building a
big ball of mud.
