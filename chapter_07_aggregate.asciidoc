...
TIP: This is optimistic concurrency control. It's optimistic because we assume
everything will be _fine_ when two users want to make changes to the database.
Pessimistic concurrency control assumes that two users will cause conflicts, so
we lock everything just to be safe.
...
An alternative to using the `REPEATABLE READ` isolation level is to use
...
scenario, `SELECT FOR UPDATE` might lead to more deadlocks, while `REPEATABLE READ`
