...
delighted by that.((("spreadsheets", "using instead of domain model"))) Business users _like_ spreadsheets because they're simple,
...
rule that restricts the possible states our model can get into.((("domain model", "invariants, constraints, and consistency")))((("invariants", "invariants, constraints, and consistency"))) An _invariant_
...
with `version=4`, and the other update will be rejected.((("transactions", "concurrent, attempting update on Product")))
...
about deadlocks).((("locks on database tables", "optimistic locking"))) With optimistic locking, you do need to explicitly handle
...
Transaction ((("isolation levels (transaction)")))((("Postgres", "documentation for transaction isolation levels")))isolation levels are tricky stuff, so it's worth spending time
...
beginning of the book to preview where we were heading?((("applications", "component diagram at end of Part One")))
...
By applying the dependency inversion principle, and((("ports", "ports-and-adapters inspired patterns")))((("adapters", "ports-and-adapters inspired patterns"))) by using the ports-and-adapters
