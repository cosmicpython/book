...
=== Events for requirements gathering

As always, let's imagine a new requirement that motivates the need to change
our architecture.  In this chapter, let's say we learn about the need to change
batch quantities when they're already in the system:  perhaps someone made a
mistake on the number in the manifest, or perhaps some sofas fell off a truck.

Events aren't just a good way of decoupling internal tasks and enforcing the
SRP in our code.  They can also be used as a way to describe your system in
conversations with the business.  A requirements-elucidation
technique called https://en.wikipedia.org/wiki/Event_storming[Event storming]
is a popular way to structure modelling conversations with the business, in
which we describe a system in terms of the events that happen in our business
processes, and the rules that surround them.

You don't need to learn anything about event storming to understand this chapter
though.  Suffice to say that, following a session with the business, we might
model the situation as in <<batch_changed_events_flow_diagram>>:
...
=== Events as an integration platform

Currently, our inputs to to the system come in the form of `POST` requests
to our API, and the only outputs is that we respond to the `allocate` API
endpoint with some JSON that includes a batch reference.

When looking to integrate with other systems, rather than having everything
call everything else via an API using synchronous HTTP protocols, we'd like
to have a more loosely coupled, atemporal solution, and the established pattern
...
TODO: WIP re-ordering this chapter as follows:
