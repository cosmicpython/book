...
+----------+    /----\      +------------+       +----------+
| Batch    |--> |RULE| -->  | Deallocate | ----> | Allocate |
| Quantity |    \----/      +------------+-+     +----------+-+
| Changed  |                  | Deallocate | ----> | Allocate |
+----------+                  +------------+-+     +----------+-+
                                | Deallocate | ----> | Allocate |
                                +------------+       +----------+
----
...
_deallocate_  those orders from that batch. Then, we want to reallocate them
to other batches (which is just the same as doing a fresh _allocate_, in fact).
...
to _deallocate_ any excess order lines, and then we can emit several events
called `Deallocated` which can be forwarded on to the existing `allocate` service,
in separate transactions.  Once again, our message bus helps us to enforce the
single responsibility principle, and it allows us to make choices about
...
* `services.allocate()` we could imagine as being the handler for an
  `AllocationRequest` event, and it can emit `Allocated` events as its output.
...
* And the new `Deallocated` events can be passed on to `services.allocate()`,
  now that it's a handler too.



All sound like a bit much?   Let's work towards it all gradually.

* We'll start by building an end-to-end test that uses redis to put
  `BatchQuantityChanged` events into the system, and look for `Allocated` events
  coming out

* We'll start by implementing a "normal" service for `change_batch_quantity()`,
  which will use our existing internal message bus to emit any `Deallocated`
  events.

* Once it's all working, we'll refactor to combine our services with our
  message handlers, so that the entire application will be a message processor.
