...
https://github.com/cosmicpython/code/tree/chapter_06_uow[chapter_06_uow] branch.
...
SQLAlchemy's own
https://docs.sqlalchemy.org/en/13/orm/session_basics.html#when-do-i-construct-a-session-when-do-i-commit-it-and-when-do-i-close-it[recommendations]:

[quote, SQLALchemy "Session Basics" Documentation]
____
the lifecycle of the session (and usually the transaction) separate and
