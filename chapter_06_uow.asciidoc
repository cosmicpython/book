...
together the repository and service layer patterns: the _Unit of Work pattern_.
...
It's a context manager that allows us to start a transaction, retrieve
...
TIP: We haven't shown it here, but it can be worth testing some of the more
    ...
    database; i.e., the same engine. For now, we're getting away with using
...
Now we'll offer a brief digression on different ways of implementing the UoW pattern.
