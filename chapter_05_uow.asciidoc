...
<<after_uow_diagram>> shows our target state: the Flask API now
only does two things: it initializes a unit of work, and it invokes
the service layer.  The service layer collaborates with the UoW, but
neither the service layer nor Flask longer needs to talk directly to the DB.
