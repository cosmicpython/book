...
//TODO (DS): Maybe the fact that it provides access to repositories needs more
//emphasis.
...
when we get to <<chapter_07_events_and_message_bus>>.
...
* It's really complementary to _Repository_ and _Service Layer_. The three of them
  together give you a nice, testable way to build almost any application in a way
  that's free of infrastructure concerns.
...
a|
* OTOH: your ORM probably already has some perfectly good abstractions around
  atomicity.  SQLAlchemy even has context managers.

* It's yet another thing to have to explain to new developers on your team
