...
.Our yield-based context manager becomes a class-based one (src/allocation/unit_of_work.py)
====
[source,python]
----
...
start = SqlAlchemyUnitOfWorkStarter
----
====

[[abstract_uow]]
.And the abstract UoW is really just documentation of intent (src/allocation/unit_of_work.py)
====
[source,python]
----
...
class _UnitOfWork(AbstractUnitOfWork):
...
here's the fake uow-starter and uow:


[[fake_uow_starter_from_abc]]
.Fake UoW starter
====
[source,python]
----
...
We have managed to deduplicate the 
`for obj in self.uow.products.seen: messagebus.handle(obj.events)` 
