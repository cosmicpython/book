...
[[email_in_services]]
.And in the services layer it's no better (services.py)
====
[source,python]
[role="non-head"]
----
...
.The service layer with an explicit message bus (services.py)
====
[source,python]
----
...
TODO: get rid of that `events[-1]` hack.  `messagebus.handle` should take
    multiple events.
...
def start(session_factory=default_session_factory):
    ...
    try:
        ...
        for obj in uow.products.seen:
            if obj.events:
                messagebus.handle(obj.events[-1])
----
====
