...
== Domain Events And A Simple Message Bus
...
<1> We'll break our rule about not using `unittest.mock.patch` for now.  We
    monkeypatch out our "real" email-sending function, which lives at
    _src/allocation/email.py_, and that'll give us a nice simple test to
    drive development.  In <<chapter_11_dependency_injection>> we'll see
    how to build out a proper abstraction for email notifications, and how
    to use DI to test it.
