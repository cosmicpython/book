...
// to make it smaller.
// EJ2: agree with above. maybe organizing in terms of a single high level scenario:
//      "A customer places an order for 2 chairs, an an order for red sofa. 
//       Each order line in an order must be allocated to a batch of stock from
//       the warehouse. If stock is not available, the purchasing department should
//       purchase a new batch, and customers should receive an ETA...."
//     Then you can just have a bullet list of rules.  I think people can fill in the blanks.
...
Python's `\___eq___` magic (or dunder) method defines the behavior of the class for the
`==` operator.
...
// (EJ1) Agree with the above points, and would hope that readers are aware.
//       I think it's adequate to point out that if you override
//       __hash__, you should also override __eq__; then wave your hands in the air,
//      warn that there are other important details, and leave a breadcrumb to 
//      something like https://www.journaldev.com/17357/python-hash-function
...
To make it work we implement `\___gt___` on our domain model:
...
// (EJ3) Manager, Builder, and Factory are code smells for very different reasons.
//  I'm not sure that calling them out here adds much, and just potentially adds to confusion. 
//  Maybe just say that not everything has to be class, unlike in some other caffeine-inspired 
//  languages.
