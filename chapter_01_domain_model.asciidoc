...
TODO: allocate single line (backport from 3)
...
def test_can_allocate_multiple_lines():
    sku1_batch = Batch('batch1', 'sku1', 100, eta=today)
    sku2_batch = Batch('batch2', 'sku2', 100, eta=today)
    sku1_line = OrderLine('oref', 'sku1', 10)
    sku2_line = OrderLine('oref', 'sku2', 10)

    allocate([sku1_line, sku2_line], [sku1_batch, sku2_batch])

    assert sku1_batch.available_quantity == 90
    assert sku2_batch.available_quantity == 90
----
====
...
[role="non-head"]
----
def allocate(lines: List[OrderLine], batches: List[Batch]) -> None:
    for line in lines:
        batch = next(
...
def test_out_of_stock_does_not_prevent_other_lines_being_allocated():
    #...

def test_multiple_lines_can_be_out_of_stock():
    #...
----
====
...
def allocate(lines: List[OrderLine], batches: List[Batch]) -> None:
    skus_out_of_stock = set()
    #...

    if skus_out_of_stock:
        raise OutOfStock(f'Out of stock for sku(s) {",".join(skus_out_of_stock)}')
----
====
