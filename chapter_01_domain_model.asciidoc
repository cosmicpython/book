...
[[test_can_allocate]]
.Testing logic for what we can allocate (test_allocation.py)
====
[source,python]
----
...
the same sku; and we've written three simple tests for a new
...
.This test is going to require a smarter model (test_allocation.py)
====
[source,python]
----
...
[[last_test]]
.Last batch test!  (test_allocation.py)
====
[source,python]
----
...
<<test_allocate_by_eta>> shows how we might test-drive such a function:

[[test_allocate_by_eta]]
.Testing our domain service (test_allocates_by_eta.py)
====
[source,python]
----
...
Python's magic methods allow us to use that nice, idiomatic `sorted()`
function; to make it work we implement `__gt__` on our domain model:
...
And we now have a domain service which we can use for our first use case.
