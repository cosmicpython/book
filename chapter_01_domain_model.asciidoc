...
// [SG] I do like an overview - thank you.
...
// [SG] Lovely explanation .. go on say it ... say ubiquitous language
...
[SG]
 I sort of get why you are not putting imports in, because then you would have to start defining a project
 structure (would that be so awful) but maybe put in
 from dataclasses import dataclass
 from typing import Optional
 from datetime import date
 or am I missing something obvious.
 Maybe worth pointing out that you are implementing the simplest solution to pass this first test -
 I presume that is what you are doing anyway as this allocate method is not checking for sku. When I
 first read this I thought that the lack of check for sku was a mistake.
...
///
[SG] so is why you are not putting imports in like
 from datetime import date
 because now you need to import Batch and OrderLine and you do not want to tell people where to put these
 in their project structure?
 from yourproject.domain.models import Batch, OrderLine
///
...
////
[SG] If the batch cannot be allocated, the allocate method just returns without complaint.
I would prefer to know if I tried to allocate to a batch and I was not able to.
Was it because I targeted the wrong batch for my sku?  Was it because the batch was exhausted?
////
...
// [SG] I love type hints but nooooooooo
...
///
[SG] Deliberate ommission of imports?
from datetime import datetime, timedelta
from yourproject.domain.models import Batch, OrderLine
from yourproject.domain.services import allocate
