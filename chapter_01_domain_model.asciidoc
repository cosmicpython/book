...
to a customer's orders. footnote:[e.j. It might be helpful to describe how the
Made functions from the viewpoint of other stakeholders (e.g., a customer ordering furniture, 
an accountant tracking disbursements, etc., or a customer service representative.
It will help make the example clearer, and drive home some of the value of DDD
and domain modeling. ]
