...
authors currently work for an online retailer of furniture.((("domain driven design (DDD)", "domain, defined"))) Depending on which system
...
The sidebar named <<allocation_notes>> shows some notes we might have taken while having a
...
line?((("domain modeling", "unit testing domain models", "dataclasses for value objects")))((("dataclasses", "use for value objects")))((("value objects", "using dataclasses for"))) In our business language, an _order_ has multiple _line_ items, where
...
change one of those values, we now have a new line.((("domain modeling", "unit testing domain models", "value objects and entities")))((("value objects", "and entities", secondary-sortas="entities"))) That's the definition of a
...
WARNING: This is tricky territory; you shouldn't modify +++<code>__hash__</code>+++ without
    ...
    "Python Hashes and Equality"] by our tech reviewer Hynek Schlawack is a good place to start.((("unit testing", "of domain models", startref="ix_UTDM")))((("domain modeling", "unit testing domain models", startref="ix_dommodUT")))
