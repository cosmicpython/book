...
to a customer's orders. footnote:[e.j. Maybe describe how 
MADE operates from the viewpoint of other stakeholders (e.g., a customer ordering furniture, 
an accountant tracking credits and debits, a customer service representative, or
the warehouse that actually has to store the stock) 
before diving into the Stock Allocation Service.  It will help home the value of DDD and domain modeling. ]
...
Time for some domain modelling.  footnote:[e.j. I find myself using collboration diagrams most frequently for these kinds of things. A sequence diagram here would be good too, but they are a little bit cumbersome to draw.  More https://www.codemag.com/Article/0205051/UML-Collaboration-Diagrams[info]. ]
...
possible, we ask for concrete examples to illustrate each rule. footnote:[e.j. Might want to have a sidebar here on an alternative modeling approach using eventstorming.]
...
#####################################
# e.j. Small style suggestion:  
# If you create locals with descriptive variable names, 
# it might be easier to grok, like this:

small_batch = Batch("batch-001", "table_sku", 20, eta=date.today())
big_batch = Batch("batch-002", "table_sku", 2, eta=date.today())
small_order = OrderLine("order-123", "table_sku", 2)
big_order = OrderLine("order-456", "table_sku", 20)
foo_order = OrderLine("order-789", "foo_sku", 1)

def test_can_allocation_if_available_greater_than_required():
    assert big_batch.can_allocate(small_order)
# And so on ...
#####################################
...
`==` operator. footnote:[e.j. The difference between "is" and "__eq__" might be
a tripping point for some people.]
...
That's lovely. footnote:[e.j. I usually prefer to use the "key" parameter
to sorted for things like this.  Then it's easier to implement more 
complex and arbitrary behaviors like "earliest, then smallest batch preferred".]
