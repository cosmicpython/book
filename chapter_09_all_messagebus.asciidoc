...
We start by defining the two events that ((("service functions", "refactoring to message handlers")))((("events and the message bus", "transforming our app into message processor", "refactoring service functions to message handlers")))capture our current API inputs:
...
Our tests now operate by creating events ((("events and the message bus", "transforming our app into message processor", "tests writtern to create events and put them on message bus")))((("testing", "tests written in terms of events")))and putting them on the
...
back to our existing handler for allocation, to be reallocated.
