...
https://github.com/cosmicpython/code/tree/chapter_09_all_messagebus[chapter_09_all_messagebus] branch on GitHub.
...
3. Our implementation will be conceptually very simple: a new
...
responsibility for putting new events onto the message bus into the message bus itself.


=== Refactoring Service Functions to Message Handlers

We start by defining the two events that ((("service functions", "refactoring to message handlers")))((("events and the message bus", "transforming our app into message processor", "refactoring service functions to message handlers")))((("BatchCreated event")))((("AllocationRequired event")))capture our current API inputs:
...
Then we rename _services.py_ to _handlers.py_; add the existing message handler
...
Along the way, we've made enriched our service-layer's API more structured and consistent. It was a scattering of
...
<3> And the UoW no longer actively puts events onto the message bus; it
...
Our API and our service layer currently((("events and the message bus", "transforming our app into message processor", "temporary hack, message bus returning results")))((("messaege bus", "returning results in temporary hack"))) want to know the allocated batch reference
...
WARNING: Whenever you split((("Unit of Work pattern", "splitting operations across two UoWs")))((("data integrity", "issues arising from splitting operation across two UoWs"))) things out like this across two units of work,
    ...
    but the second does not. You'll need to think about whether this is acceptable,
...
definitely trigger our spidey sense. <<chapter_11_external_events>> and <<epilogue_1_how_to_get_there_from_here>> have some tips
...
on `FakeUnitOfWork`, and decouple it from the real message bus, instead making
...
going to come from? The answer is revealed in a couple of chapters' time.((("events and the message bus", "transforming our app into message processor", startref="ix_evntMBMP")))  But
