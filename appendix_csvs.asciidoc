...
[[first_cut_csvs]]
.A first cut of our CSV reader/writer (allocate-from-csv)
====
[source,python]
[role="non-head"]
----
...
def main(folder):
    ...
    with orders_path.open() as inf, allocations_path.open('w') as outf:
        ...
        writer.writerow(['orderid', 'sku', 'batchid'])
        for row in reader:
            orderid, sku = row['orderid'], row['sku']
            qty = int(row['qty'])
            line = model.OrderLine(orderid, sku, qty)
            batchid = model.allocate(line, batches)
            writer.writerow([line.orderid, line.sku, batchid])
...
class CsvRepository:
    ...
    def _load(self):
        ...
        with self._allocations_path.open() as f:
            reader = csv.DictReader(f)
            for row in reader:
                batchid, orderid, sku = row['batchid'], row['orderid'], row['sku']
                qty = int(row['qty'])
                line = model.OrderLine(orderid, sku, qty)
                batch = self._batches[batchid]
...
class CsvUnitOfWork:
    ...
    def commit(self):
        with self.batches._allocations_path.open('w') as f:
            writer = csv.writer(f)
            writer.writerow(['orderid', 'sku', 'qty', 'batchid'])
...
[[final_cli]]
.Allocation with CSVs in 9 lines (allocate-from-csv)
====
[source,python]
----
def main(folder):
    orders_path = Path(folder) / 'orders.csv'
    start_uow = lambda: nullcontext(csv_uow.CsvUnitOfWork(folder))
