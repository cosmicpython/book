...
the right-hand side of the graph looks like, but the hints are there.  Think
about what our domain model would look like if we were using the Django ORM
for example.  the `._allocations` attribute on `Batch` would have to change from
being a set to being some sort of foreign key / related field, and Django would
start demanding that any objects we add to it have a valid id/pk field, making
unit testing that little bit more tedious.
