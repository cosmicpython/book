...
separate (which is a good thing), and to have each layer depends only on the one
below...
...
separated from storage concerns when our model properties are directly coupled
to database columns?
...
complexity locally, and it has a cost in terms of moving parts and
ongoing maintenance.
...
// this sentence looks weird ^ , is the 1st "are" the correct word?
