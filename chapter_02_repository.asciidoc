...
as quickly as possible.  In our case that's going to be a web API.
...
starting to look ugly as hell. Can we really say this model is persistence
ignorant?
...
Instead, we could just use our objects whenever we liked. What would tht look
like?
...
def create_a_batch(self):
    batch = Batch(...)
    all_my_data.batches.add(batch) #<1>

def modify_a_batch(self, batch_id, new_quantity):
    batch = all_my_data.batches.get(batch_id)
    batch.change_initial_quantity(new_quantity) #<2>
