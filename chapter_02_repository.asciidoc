...
some of the pros and cons of _Repository Pattern_.


[[chapter_02_repository_tradeoffs]]
[options="header"]
.Repository Pattern: The Trade-Offs
|===
|Pros|Cons
a|
* we have a simple interface between persistent storage and our domain model
* it's well know for developers coming from outside the Python world
* by reducing the API surface around what you can do to "save" models, we've
  reduce the likelihood of weird things happening
* it's easy to make a fake version of the repository for unit testing
* and it's easy to swap out different storage solutions, we've fully decoupled
  the model from infrastructure concerns
a|
