...
a way of decoupling our core logic from infrastructural concerns.((("Repository pattern", id="ix_Repo")))

We'll introduce the _Repository_ pattern, a simplifying abstraction over data storage,
allowing us to decouple our model layer from the data layer.((("data storage", "Repository pattern and"))) We'll present a
...
https://github.com/cosmicpython/code/tree/chapter_02_repository[chapter_02_repository] branch on GitHub.
...
In <<chapter_01_domain_model>>r we built a simple domain model that can allocate orders
to batches of stock.((("Repository pattern", "persisting the domain model")))((("domain modeling", "persisting the domain model"))) It's easy for us to write tests against this code because
...
As mentioned in <<introduction>>, a layered architecture is ((("data access", "applying dependency inversion principle to")))((("layered architecture")))a common
...
"inside," and dependencies flowing inward to it; what people sometimes call
...
> Is this ports and adapters? Or is it hexagonal architecture? Is that the same
> as onion architecture? What about the clean architecture? What's a port,
...
Let's remind ((("domain model", id="ix_domod")))((("Repository pattern", "domain model recap")))ourselves of our domain model (see <<model_diagram_reminder>>):
...
Well, thankfully, that's not the only way to use SQLAlchemy.((("data model", "translating to relational database", "ORM depends on the model")))((("dependency inversion principle", "ORM depends on the data model")))((("SQLAlchemy", "explicit ORM mapping with SQLAlchemy Table objects")))((("classical mapping")))  The alternative is
...
maintainers, and Mike Bayer in particular.] As so often happens with
...
The _Repository pattern_ is an abstraction over persistent storage.((("domain model", startref="ix_domod"))) It hides the
...
An alternative to look into are https://www.python.org/dev/peps/pep-0544/[PEP
544 protocols]. These give you typing without the possibility of inheritance,
...
The _Repository pattern_ is probably one of the easiest choices in the book, though,
...
Now Python doesn't have interfaces per se, so although it's
...
expects-- he function and method names in use, and their argument names and types.
