...
<2> `raise NotImplementedError` is nice but neither necessary nor sufficient.
    ...
    can call out to, if you want.footnote:[Many smart people will tell you that
putting behavior into ABCs is bad design; "prefer composition over inheritance",
and all that jazz. We've done it a few times in this book, for simplicity, but
that doesn't mean you have to.  If you want typing without the possibility of
inheritance, check out https://www.python.org/dev/peps/pep-0544/[PEP 544 protocols].]
