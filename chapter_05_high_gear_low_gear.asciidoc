...
our service-layer functions.((("domain layer", "fully decoupling service layer from", id="ix_domlaydec")))((("service layer", "fully decoupling from the domain", id="ix_serlaydec")))
...
we see fit.((("domain layer", "fully decoupling service layer from", startref="ix_domlaydec")))((("service layer", "fully decoupling from the domain", startref="ix_serlaydec")))
...
Thanks to our service function, adding the endpoint is easy, just a little
...
of your test coverage to unit tests and develop a healthy test pyramid.  A few
...
Error handling counts as a feature::
    ...
    This means you need to test only the happy path for each feature, and
