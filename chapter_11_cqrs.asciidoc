...
A set of fully normalised relational tables is a good way to make sure that
write operations never cause data corruption.  But retrieving data using lots
of JOINs can be slow.  It's common in such cases to add some denormalised views
...
made it nicer by keeping a totally separate, denormalised datastore for our
view model?
