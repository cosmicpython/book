...
[quote, Our Readers]
____
What?  Are you guys on _drugs_?
____

Good question.  Why would we want to do this?  Well, let's explore some more
sane-seeming alternatives first. 
...
<1> We're sure you can figure out how to write a query called `.for_order()`
    on your repository that can find all the products for an order.

<2> The repository gives us back products but we want batches, so we
    get them with a list comprehension.

<3> And then we filter again to get just the batches for our specific
    order.

So, yes, on the plus side we're re-using our existing database access abstraction.
But on the downside, we're doing a lot of manual looping and filtering in Python,
which is slow and something you'd usually want your database to do for you.

And there's even more Python-looping: we need to add a new `@property` to the
domain model to be able to get the order ids allocated to a batch:
...
reads are sometimes conceptually quite different.
...
`Products` is clunky, then I can just use my ORM and work with `Batches`.
...
it first seemed?  Perhaps we were exaggerating the craziness for effect?

Just you wait.

So. Crazy or not, that hardcoded SQL query is pretty ugly right?  What if we
made it nicer by _keeping a totally separate, denormalized datastore for our
view model?_
...
==== Updating a View Model Table Using an Event Handler
...
And believe it or not, that will pretty much work!  _And it will work
...
Speaking of which, let's talk about testing.  Whichever of the approaches you
decide to go for, the most obvious kind of test to write is an integration
test, one that goes to a real database.
