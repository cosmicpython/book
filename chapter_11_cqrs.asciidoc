...
can have. If you're struggling to scale a system with a complex data store, ask
whether you could build a simpler read model.

TODO: Diagram (cmd -> handler (single DB instance) -> event; event -> handler (multiple db instances); view -> read instances )
// Do we need this? It might actually be covered by the seuqnce diagram futher
down

Keeping them up to date is the challenge!  Postgres views and triggers are a
