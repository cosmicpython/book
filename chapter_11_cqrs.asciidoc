...
=== Async Post  and Post/Redirect/Get

If you've used REST APIs, you're probably familiar with the
Async Post or Post/Redirct/Get patterns.

Async Post is a technique where a HTTP endpoint accepts an
HTTP POST, and responds with a 202 Accepted, with resource link to see the result.
For example, we might accept a POST to `/batches` to create a new batch, and
return a link to `/batches/123`, where the client can retrieve their newly created batch.

Async Post fixes the problem where a POST may trigger a long running operation,
and we do not want to block the user while waiting on the result

When used together with a Redirect, it's called Post/Redirect/Get.
Post/Redirect/Get fixes the problems that arise when users refresh the results page
...
All these problems happen because we're returning data in response to a write
operation. Async Post and Post/Re-direct/Get side-step the issues by separating the read and
write phases of our operation.

These techniques are simple examples of CQS - Command-Query Separation. In CQS we
