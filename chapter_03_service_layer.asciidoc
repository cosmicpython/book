...
[role="skip"]
----
def allocate(line: OrderLine, repo: AbstractRepository<1>, session) -> str:
----
====

<1> It depends on a repository.  We've chosen to make the dependency explicit,
    ...
    decided that they help. What you decide to do in your own codebase is up to
    you.]
