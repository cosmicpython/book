...
ever storing it. pass:[<span class="keep-together">Getting</span>] the timing right is a tricky balancing act when goods take
...
(whisper it) _enterprise_ pass:[<span class="keep-together"><em>software</em></span>].
...
Enterprise Application Architecture_ (both published by Addison-Wesley pass:[<span class="keep-together">Professional</span>])—which we often refer to and
...
Domain modeling and DDD (Chapters <<chapter_01_domain_model,1>> and <<chapter_02_repository,2>>)::
