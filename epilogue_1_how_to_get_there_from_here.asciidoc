...
We used event interception to move from this:
...
TIP: Check out www.eventmodelling.org[] and www.eventstorming.org[] for some great
...
Do I need to do all of this at once? Can I just do a bit at a time?::
    ...
    have an existing system, we recommend building a service layer to try to keep orchestration
    in one place. Once you have that, it's much easier to push logic into the model
    and push edge concerns like validation or error handling to the entrypoints.
    +++<br />+++
...
Extracting use cases will break a lot of my existing code; it's too tangled::
    Just copy and paste. It's OK to cause more duplication in the short term. Think of
    this as a multistep process. Your code is in a bad state now, so copy and paste
    ...
    new code and finally delete the mess. Fixing large codebases is a messy and
...
Do I need to do CQRS? That sounds weird. Why can't I just use repositories?::
    ...
    to punish yourself.
    +++<br />+++
    In our first case-study system, we had a lot of _View Builder_ objects that used
...
How should use cases interact across a larger system? Is it a problem for one to call another?::
    ...
    bunch of subdomains that center on a particular aggregate or set of
...
Is it a smell for a use case to use multiple repositories/aggregates, and why?::
    ...
    consistency boundary is wrong, strictly speaking. Ideally you should think
    about moving to a new aggregate that wraps up all the things you want to
    change at the same time.
    +++<br />+++
    If you're actually updating only one aggregate and using the other(s) for
    read-only access, then that's _fine_, although you could consider building a
    ...
    https://dddcommunity.org/wp-content/uploads/files/pdf_articles/Vernon_2011_1.pdf[these papers on aggregate design]
    by Vaughn Vernon.
...
What if I have a read-only but business-logic-heavy system?::
    ...
    +++<br />+++
    We've written systems in which the view models needed extensive unit tests. In
    those systems, we split a _view builder_ from a _view fetcher_, as in
...
This makes it easy to test the view builder by giving it mocked data; e.g., a list
of dicts.
+++<br />+++
 ...
 * https://bravenewgeek.com/you-cannot-have-exactly-once-delivery/[You cannot have exactly-once delivery].
 * https://bravenewgeek.com/what-you-want-is-what-you-dont-understanding-trade-offs-in-distributed-messaging/[Understanding tradeoffs in distributed messaging].
...
* https://blog.sapiensworks.com/post/2015/08/26/How-To-Ensure-Idempotency[How to ensure idempotency]
* https://lostechies.com/jimmybogard/2013/06/03/un-reliability-in-messaging-idempotency-and-de-duplication/[Un-reliability in messaging]
...
there is other prior art. Greg Young wrote an entire book on https://www.goodreads.com/book/show/34327067-versioning-in-an-event-sourced-system[managing event-driven systems over time].
...
* https://martinfowler.com/books/eip.html[Enterprise Integration Patterns] by
  Hohpe and Woolf is a pretty good start for messaging patterns.
...
Phew! That's a lot of warnings and further reading suggestions, we hope we
haven't scared you off completely. Our goal with this book has been to give you
just enough knowledge and intuition that you can start building some of this
