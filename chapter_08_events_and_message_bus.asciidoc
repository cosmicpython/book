...
<1> We commit even if we fail to allocate because the code is simpler this way
...
Events are useful for more than just sending emails, though. In <<chapter_05_high_gear_low_gear>> we
...
In <<chapter_08_events_and_message_bus>>, we'll look at this idea in more detail as we build a more complex
