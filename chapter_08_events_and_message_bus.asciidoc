...
Then we'll show how to use _Domain Events_ pattern to separate side effects from our((("Domain Events pattern")))((("Message Bus pattern")))
...
So. Email alerts when we run out of stock. ((("email", "sending email alerts when out of stock", id="ix_email")))((("events and the message bus", "sending email alerts when out of stock", id="ix_evntMBeml")))When we have a new requirement like
this, which _really_ has nothing to do with the core domain, it's all too easy to
start dumping these things into our web controllers.((("web controllers", "sending email alters via, avoiding")))
...
to follow to achieve a goal.((("events and the message bus", "sending email alerts when out of stock", "out of place in the service layer")))((("service layer", "sending email alerts when out of stock, avoiding")))
...
this code?((("events and the message bus", "sending email alerts when out of stock", startref="ix_evntMBeml")))((("email", "sending email alerts when out of stock", startref="ix_email")))
...
are all called pass:[<code><span class="keep-together">allocate</span></code>], not `allocate_and_send_mail_if_out_of_stock`.
...
recording _events_—facts about things that have happened.((("events and the message bus", "recording events"))) We'll use a message
...
Handlers are _subscribed_ to receive events, which we publish to the bus.((("publish-subscribe system, message bus as"))) It
...
handlers whenever an event happens.((("data model", "events from, passing to message bus in service layer")))((("message bus", "service layer with explicit message bus")))((("events and the message bus", "service layer with explicit message bus")))((("service layer", "taking events from model and putting them on message bus"))) Now all we need is to connect the two. We
...
can do better?((("repositories", "TrackerRepository wrapper class")))((("composition over inheritance", "TrackingRepository wrapper class")))((("inheritance", "avoiding use of with wrapper class")))
...
Switching all ((("abstract base classes (ABCs)", "switiching to typing.Protocol")))the ABCs to `typing.Protocol` is a good way to force yourself to avoid using inheritance. Let us know if you come up with something nice!
******************************************************************************

=== Wrap-Up

Domain events give us a way to handle workflows in our system.((("events and the message bus", "domain events and message bus wrapup"))) We often find,
...
we have the tools we need to answer the question.((("aggregates", "changing multiple aggregates in a request")))
