...
TODO: talk about pythonic duck typing, interfaces / abstract classes in OO,
    and our plan to use ABCs
...
TODO: we've explained what, and we'll explain how, but we need to spend a moment
    on _why_ dependency inversion is a good idea.  broadly, because we want to be
    able to change them independently of each other.  high-level modules we want
    to make as easy to change as possible in response to business need.  low-level
    modules / details are often harder to change (think, database migrations vs
    changing a function name), but it's also important to _be able_ to change
    them when you need (eg sharding a database)
