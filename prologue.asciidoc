...
using the first sense. We encapsulate behaviour by identifying a task that needs
to be done in our code, and giving that task to a well defined object or function.

////
TODO: (EJ)
Formally we call these tasks _responsibilities_ or _behaviours_ and an object that
is responsible for a behaviour is said to have a _role_ in the system.

NOTE: We didn't invent this approach, it comes from Responsibility-Driven Design
    - an OO philosophy from the late Eighties. If you want to read more about it
    you should check out the book _Object Design: Roles, Responsibilities, and
    Collaborations_, by Rebecca Wirfs-Brock and Alan McKean (Addison-Wesley 2003)

This is a behaviour-driven approach. Instead of building classes around the
data in our system, we're going to build classes around the processes that need
to happen.
////
...
// TODO (EJ) Responsibility" might be the better term here. Here's http://www.wirfs-brock.com/Design.html[the origin of roles in OOP. ]]
