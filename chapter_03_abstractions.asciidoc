...
tests, and how do we change our implementation to make it all work?((("filesystems", "writing code to synchronize source and target directories", "implementing chosen abstraction", id="ix_filesyncimp")))
...
driving it with direct unit tests.((("testing", "after implementing chosen abstraction", "edge-to-edge testing with fakes and dependency injection", id="ix_tstaftabsedge"))) At some point, though, we want to test bigger
...
enumerating tests for all the edge cases is now much easier.((("dependencies", "edge-to-edge testing with dependency injection", startref="ix_depinj")))((("testing", "after implementing chosen abstraction", "edge-to-edge testing with fakes and dependency injection", startref="ix_tstaftabsedge")))((("edge-to-edge testing", startref="ix_edgetst")))
...
We'll see this idea come up again and ((("testing", "after implementing chosen abstraction", startref="ix_tstaftabs")))((("filesystems", "writing code to synchronize source and target directories", "implementing chosen abstraction", startref="ix_filesyncimp")))again in the book: we can make our
