...
////
[SG] This locally coupled code is often refered to as "strongly cohesive" right?  Is it worth
using this existing term so that people can more easily recognise ideas they have heard about before
////
...
////
[SG] In the text "there is a high degree of coupling between the two because of reasons"
...
to the smallest part of the problem, and then interactively make the solution
...
// [SG] missing import tempfile for these tests?
...
////
[SG] In my head this is you defining the interface, the contract that will have to be implemented
////
...
////
[SG] This makes it sound like your only motivation for refactoring is so that you
can test more easily - maybe it is? Is TDD this extreme?
////
...
// [SG] What is the top-level module - is this the sync funtion?
...
////
[SG] I can see this tests determine_actions() but what happened to the test of sync code?  Isn't sync the
only real public method in the module?  Is determine_actions going to be public - I am consfused,
are you throwing sync away...?
////
...
////
[SG]  typos(?) in the code snippet above:
source_hashes -> src_hashes (or vice versa)
dest_hashes  ->  dst_hashes
olddestpath -> oldestpath
filesystem.del -> filesystem.delete(*)
////
...
\\[SG] how did we get to this implementation - what happened to the sync function
...
At this point some of you will be scratching your head and thinking
