...
(HP) this is a good question.  I don't _think_ that's quite what we're trying to
do, or not quite the lens we want to put on it.  If the only tests we needed were
the two above, then we'd be fine.  But the thing is, our current implementation
is full of bugs, we need to write lots more tests to get it working, and doing
so with the current cumbersome setup is a pain.  And it would be even more of a
pain if we added new requirements like `--dry-run` or something.

TODO: write this up somewhere.
////
...
decide that's fine because `sunc()` is now so simple.  Or you might decide to
