...
// [KP] Again, one could mention PEP544 protocols here. For some reason, I like them.
...
////
[KP] I used to be guilty of over-mocking, myself. However, I occasionally find
it useful to test a scenario where I do not want to do a complete setup of
a particular state and just let some "is_a_thing()" method return True/False.

The reason I find this OK may be due to the high complexity it takes to set up test
scenarios with Django models, though.
////
